...
const DATABASE_ID = '10d8863b87d482618cca8124788eed07'
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Builds sync: found ${allResults.length} updates (full=${fullSync}, incremental=${useIncremental})`)
    ...
    for (const page of allResults) {
      ...
      let htmlContent = ''
      try {
        htmlContent = await renderBlogContent(page.id, NOTION_API_KEY)
        const mediaResult = await persistMediaInHtml(supabase, supabaseUrl, htmlContent, `build-${slug}`)
        htmlContent = mediaResult.html
        totalMediaPersisted += mediaResult.count
      } catch (e) {
        console.error(`Failed to render build update "${title}":`, e)
      }
      ...
      const row = {
        ...
        version: properties['Version']?.rich_text?.[0]?.plain_text || null,
        change_types: (properties['Change Type']?.multi_select || []).map((o: any) => o.name),
      ...
      await supabase.from('build_updates_cache').upsert(row, { onConflict: 'notion_id' })
    }
    ...
    try {
      ...
      const { data: existing } = await supabase.from('build_updates_cache').select('notion_id')
      const stale = (existing || []).map(r => r.notion_id).filter(id => !liveIds.has(id))
      if (stale.length > 0) {
        await supabase.from('build_updates_cache').delete().in('notion_id', stale)
        prunedCount = stale.length
        console.log(`Pruned ${prunedCount} stale build updates: ${stale.join(', ')}`)
      }
    } catch (e) {
    ...
    await supabase.from('sync_metadata').upsert(
      { sync_type: 'builds', last_synced_at: syncStartTime },
      { onConflict: 'sync_type' }
    )


    console.log(`Builds sync complete: ${allResults.length} updates, ${totalMediaPersisted} media files persisted`)
