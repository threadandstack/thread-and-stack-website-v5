...
const ServicesCarousel = () => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") setSelected((s) => (s + 1) % services.length);
      if (e.key === "ArrowLeft") setSelected((s) => (s - 1 + services.length) % services.length);
    };
  ...
  return (
    <div
      ref={stageRef}
      className="relative my-6 h-[640px] overflow-hidden [perspective:1800px] select-none md:my-0 md:h-[600px] md:overflow-visible"
      ...
      onTouchEnd={(e) => {
        ...
        if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)) {
          if (dx < 0) setSelected((s) => (s + 1) % services.length);
          else setSelected((s) => (s - 1 + services.length) % services.length);
        }
      }}
    >
        ...
        {services.map((s, index) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <h3 className="font-serif-pro italic text-2xl md:text-3xl tracking-tight">{s.name}</h3>
                <div
                  className="mt-2 text-[34px] font-medium leading-none tracking-[-0.03em] md:text-[40px] bg-clip-text text-transparent"
              ...
              {isActive ? (
                ...
                <span className="mt-auto inline-flex h-11 items-center justify-center rounded-md border border-hairline px-5 text-[13.5px] font-medium text-foreground/70">
                  Tap to focus
                </span>
              )}
            </button>
      ...
      <button
        type="button"
        onClick={() => setSelected((s) => (s - 1 + services.length) % services.length)}
        aria-label="Previous"
      ...
      <button
        type="button"
        onClick={() => setSelected((s) => (s + 1) % services.length)}
        aria-label="Next"
